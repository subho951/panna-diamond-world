...
class LeadListController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $query = BranchLead::query();  // Query Builder
        ...
        if(!empty($request->input('telecaller')))
        {
            $selected_telecaller_id = Helper::decoded($request->input('telecaller'));                              
            $data['selected_telecaller_id'] = $selected_telecaller_id ;
    
            $query->where('assigned_telecaller_id', '=', $selected_telecaller_id);
        }

        if(!empty($request->input('parent-status')) || !empty($request->input('child-status')))
        {
            $selected_parent_status_id = "";
            $selected_child_status_id = "";

            if(!empty($request->input('parent-status')))
            {
            ...
            if(!empty($request->input('child-status')))
            {
            ...
            {
                $query->where('parent_status_id', '=', 0)->where('child_status_id', '=', 0);
            }
            elseif($selected_parent_status_id == 12)
            {
                $query->where('parent_status_id', '=', 0);
            }
            elseif($selected_child_status_id == 13)
            {
                $query->where('child_status_id', '=', 0);
            }
            elseif(!empty($selected_parent_status_id) || !empty($selected_child_status_id))
            {
                $query->whereHas('leadActivities', function ($qry) {
                    $qry->where('status', '!=', 3);
                })
                ->when(!empty($selected_parent_status_id) || !empty($selected_child_status_id), function ($qry) use ($selected_parent_status_id, $selected_child_status_id) {
                    $qry->whereIn('lead_sl_no', function ($subQuery) use ($selected_parent_status_id, $selected_child_status_id) {
        ...
        if(session('user_data')['role_id'] == 3) // for telecaller
        {
            ...
            $query->where('assigned_telecaller_id', '=', $this_telecaller_id);
        }    
