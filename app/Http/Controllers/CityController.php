...
class CityController extends Controller
{
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'city.add-edit';
            $data['row']                    = City::where('id', '=', $id)->first();
            $country_id                     = (($data['row'])?$data['row']->country_id:0);
            $data['couns']                  = Country::select('id', 'name')->where('status', '=', 1)->orderBy('name', 'ASC')->get();
            $data['sts']                    = State::select('id', 'name')->where('status', '=', 1)->where('country_id', '=', $country_id)->orderBy('name', 'ASC')->get();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                    'country_id'            => 'required',
