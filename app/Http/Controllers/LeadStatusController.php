...
class LeadStatusController extends Controller
{
    ...
    /* add */
        public function add(Request $request){
            ...
            $page_name                      = 'lead-status.add-edit';
            $data['row']                    = [];
            $data['parent_Stats']           = LeadStatus::select('id', 'name')->where('status', '=', 1)->where('parent_id', '=', 0)->orderBy('name', 'ASC')->get();
            $data                           = $this->siteAuthService ->admin_after_login_layout($title,$page_name,$data);
            return view('maincontents.' . $page_name, $data);
        }
    /* add */
    /* edit */
        public function edit(Request $request, $id){
            ...
            $page_name                      = 'lead-status.add-edit';
            $data['row']                    = LeadStatus::where('id', '=', $id)->first();
            $data['parent_Stats']           = LeadStatus::select('id', 'name')->where('status', '=', 1)->where('parent_id', '=', 0)->orderBy('name', 'ASC')->get();
            if($request->isMethod('post')){
                $postData = $request->all();
                $rules = [
                ...
                if($this->validate($request, $rules)){
                    ...
                    LeadStatus::where($this->data['primary_key'], '=', $id)->update($fields);
                    /* user activity */
                        $activityData = [
                            'user_email'        => session('user_data')['email'],
                            'user_name'         => session('user_data')['name'],
