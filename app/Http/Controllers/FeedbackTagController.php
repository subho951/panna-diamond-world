...
class FeedbackTagController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data = array(
            'title'             => 'Feedback Tag',
            'controller'        => 'FeedbackTagController',
            'controller_route'  => 'feedback-tag',
            'primary_key'       => 'id',
            'table_name'        => 'feedback_tags',
        );
    }
    ...
    public function add(Request $request)
    {
        ...
        $page_name                      = 'feedback-tag.add-edit';
        $data['row']                    = [];
        $data                           = $this->siteAuthService ->admin_after_login_layout($title,$page_name,$data);
        return view('maincontents.' . $page_name, $data);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $page_name                      = 'feedback-tag.add-edit';
        $data['row']                    = FeedbackTag::where($this->data['primary_key'], '=', $id)->first();
        ...
        if($request->isMethod('post'))
        {
            ...
            if($this->validate($request, $rules))
            {
                ...
                FeedbackTag::where($this->data['primary_key'], '=', $id)->update($fields);
                /* user activity */
                    $activityData = [
                        'user_email'        => session('user_data')['email'],
                        'user_name'         => session('user_data')['name'],
